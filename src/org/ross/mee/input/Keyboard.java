...
public class Keyboard 
{
	...
	private static String last_char;
	private static boolean new_char_invoke;
	private static boolean new_key_invoke;
	private static  int last_action;
	private ArrayList<InputEvent> keyboard_triggers;
	
	public Keyboard()
	{
		keyboard_triggers=new ArrayList<InputEvent>();
		last_key=-1;
		last_action=-1;
		last_char="";
		new_char_invoke=false;
		new_key_invoke=false;
		glfwSetKeyCallback(Window.getID(), new GLFWKeyCallbackI()
		{
		...
		glfwSetCharCallback(Window.getID(), new GLFWCharCallback() 
		{
			...
			{
				last_char = Character.toString((char)c);
				new_char_invoke=true;
			}
		});
	}
	...
	public void Process()
	{
		for (int i=0; i<keyboard_triggers.size(); i++)
		{
			if (last_action==GLFW_PRESS && new_key_invoke)
			{
				keyboard_triggers.get(i).onKeyPressed(last_key);
			}
			if (last_action==GLFW_RELEASE && new_key_invoke)
			{
				keyboard_triggers.get(i).onKeyReleased(last_key);
			}
		}
	...
	{
		if (last_key==key && last_action==GLFW_PRESS)
		{
			return true;
		}
		return false;
	...
	{
		if (last_key==key && last_action==GLFW_RELEASE)
		{
			return true;
		}
		return false;
	...
	public void removeEvent(String name)
	{
		for (int i=0; i<keyboard_triggers.size(); i++)
		{
			if (keyboard_triggers.get(i).getTriggerName()==name)
			{
				keyboard_triggers.remove(i);
			}
		}
	}
	...
	public void removeEvent(InputEvent event)
	{
		for (int i=0; i<keyboard_triggers.size(); i++)
		{
			if (keyboard_triggers.get(i)==event)
			{
				keyboard_triggers.remove(i);
			}
		}
